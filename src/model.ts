...
export type Classification = { matches: Record<string, { path: string; parts: string[] }[]>; folders: string[][]; canUndo: boolean }
export type Boot = { destination?: string; classification?: Classification; document: Catalog; files: string[]; folder: string; dataDir: string; context: string; dates: Record<string, { modified: number; created: number }> }
